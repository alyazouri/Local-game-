// =====================================================
//  JO_PUBG_JO_LOCK.pac - النسخة النهائية المطورة
//  - حصر PUBG ضمن النطاقات IPv6 Jordan (JO) المحددة
//  - منع أي IPv6 literal يخرج عن النطاقات الأردنية
...
// =====================================================
// 2) JO_V6_INTERVALS — النطاقات المسموحة الأردنية
// ملاحظة: تم دمج النطاقات "القوية" و "العادية" في منطق المعالجة
// =====================================================
...
// دمج المتغيرات لتجنب التكرار في المنطق (isStrongJOIPv6 لن تُستخدم في الكود الرئيسي)
...
function isStrongJOIPv6(ip){
    return isJOIPv6(ip); // تبسيط المنطق
}
...
function FindProxyForURL(url,host){
    ...
    // 1) الترافيك المحلي/الخاص يذهب DIRECT (تخفيف الحمل على البروكسي)
    if (isPrivateOrLocal(host))
        return "DIRECT"; 

    var ipv6   = extractIPv6Host(host);
    var isGame = isPubgTraffic(url,host);
    var jo     = ipv6 ? isJOIPv6(ipv6) : false; // للتحقق من النطاق الأردني

    // 2) منع أي IPv6 literal يخرج عن النطاقات الأردنية
    ...
    if (isGame){
        ...
        // إذا كان الاتصال ضمن النطاقات الأردنية (جو)
        if (jo) {
             // استخدم وضع اللعب الذي يناسب الـ URL (MATCH, LOBBY,...)
             return buildProxy(mode); 
        }

        // إذا كان ترافيك PUBG ولكنه ليس على IPv6 أردني 
        // المبدأ العام هو توجيهه عبر البروكسي، الذي يُفترض أنه في الأردن (37.220.120.16)
        return buildProxy(mode);
    }

    // 4) أي شيء غير PUBG (باستثناء المحلي والمستثنى) → بروكسي لابي (443)

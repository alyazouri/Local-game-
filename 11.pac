// JO-HARD-MODE PAC — v6 (strict, optimized, hardened)
// هدف: أعلى احتمال للّوبي/الماتش الأردني + حماية أعلى + أداء أفضل.
// أي خروج يكون عبر بروكسي أردني فقط (لا يوجد DIRECT في الوضع الطبيعي).
...
// بادئات IPv6 الأردنية — شاملة لكل الفئات (كما في v5)
var JO_V6_PREFIX = {
  LOBBY: [
    "2a03:6b01:1000::/34",
    "2a03:6b01:8000::/34",
...
// TTLs & كاش
var DNS_TTL_MS = 30*1000;        // DNS cache 30 ثانية
var PROXY_STICKY_TTL_MS = 90*1000; // تثبيت البروكسي 90 ثانية
...
C._INIT_DONE = C._INIT_DONE || false;
C._JO_V4I = C._JO_V4I || [];
C._JO_V6C = C._JO_V6C || {};
...
function rangePair(a){
  return{s:ip4ToInt(a[0]),e:ip4ToInt(a[1])};
}

// تهيئة JO_V4_RANGES مرة واحدة إلى أعداد صحيحة
function initV4(){
...
// تهيئة JO_V6_PREFIX مرة واحدة إلى بنية جاهزة
function initV6(){
  if(C._JO_V6C && C._JO_V6C._ready) return;
  C._JO_V6C = {};
  for(var cat in JO_V6_PREFIX){
    if(!JO_V6_PREFIX.hasOwnProperty(cat)) continue;
    var arr = JO_V6_PREFIX[cat];
    var out = [];
    for(var i=0;i<arr.length;i++){
      out.push(parseCidr6(arr[i]));
    }
    C._JO_V6C[cat]=out;
  }
...
function isJOv6ForCat(ip,cat){
  ...
  initV6();
  var arr=C._JO_V6C[cat];
  if(!arr || !arr.length) return false;
  for(var i=0;i<arr.length;i++){
    if(match6(ip,arr[i])) return true;
  }
  return false;
}
...
// منع dotless/local/أسماء مش طبيعية
function isUnsafeHost(h){
  if(!h) return true;
  if(isPlainHostName(h)) return true;          // بدون نقطة
  if(shExpMatch(h,"*.local") || shExpMatch(h,"*.lan")) return true;
  // أسماء بأرقام فقط بلا نقطة → اعتبرها غير آمنة
...
// قرار الفئة مع كاش (لتحسين السرعة)
function enforceCat(cat, host){
  ...
  if(isIp6Literal(ip) && isJOv6ForCat(ip,cat)){
    res = proxyFor(cat);
    C.enforceCache[key]=res;
    return res;
  }
  if(isIp4(ip) && isJOv4(ip)){
    res = proxyFor(cat);
    C.enforceCache[key]=res;
    return res;
  }
...
function FindProxyForURL(url, host){
  ...
  // 4) غير ذلك: كل شيء يطلع عبر البروكسي الأردني (بدون DIRECT)
  return enforceCat("LOBBY", host);
}
